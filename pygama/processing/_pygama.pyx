...
def SimulateTier0( filename, output_file_string = "t1", chan_list=None, n_max=np.inf, verbose=False, output_dir=None, decoders=None):
  '''
    Reads in fake "raw," or "tier 0," siggen-like data and saves to a hdf5 format using pandas
  ...
  # pull out the run number
  runNumber = -1 #get_run_number(headerDict)
  print("Run number: {}".format(runNumber))

  # id_dict = get_decoder_for_id(headerDict)
  id_dict = {1:'SimulatedWaveformDigitizerDecoder'}
...
#  id_to_decoder = id_dict
  ...
  t1_file_name = os.path.join(output_dir, output_file_string+'_sim{}.h5'.format(runNumber))
